...
public class CommonWebpageDAO extends IDAO<Webpage> {
	private final static String INDEX_COMMONS = "commons", TYPE_WEBPAGE = "webpage";
	...
	protected boolean check() {
		return esUtil.checkIndex("commons", "mapping/commonIndex.json")
				&& esUtil.checkType("commons", "webpage", "mapping/webpage.json");
	}
	...
	public String index(Webpage webpage) {
		...
		try {
			indexResponse = client.prepareIndex(INDEX_COMMONS, TYPE_WEBPAGE).setSource(gson.toJson(webpage)).get();
	...
	public List<Webpage> getWebpageBySpiderUUID(String uuid, int size, int page) {
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(QueryBuilders.matchQuery("spiderUUID", uuid)).setSize(size).setFrom(size * (page - 1));
	...
	private void exportTitleContentPairBy(QueryBuilder queryBuilder, OutputStream outputStream) {
		esUtil.exportData(queryBuilder, searchResponse -> {
			...
			return resultList;
		}, outputStream, INDEX_COMMONS, TYPE_WEBPAGE);
	}
	...
	private void exportWebpageJSONBy(QueryBuilder queryBuilder, Boolean includeRaw, OutputStream outputStream) {
		esUtil.exportData(queryBuilder, searchResponse -> Lists.newLinkedList(), searchResponse -> {
			List<String> resultList = Lists.newLinkedList();
			List<Webpage> webpageList = warpHits2List(searchResponse.getHits());
			webpageList
					.forEach(webpage -> resultList.add(gson.toJson(includeRaw ? webpage : webpage.setRawHTML(null))));
			return resultList;
		}, outputStream, INDEX_COMMONS, TYPE_WEBPAGE);
	}
	...
	public void exportWebpageJSONBySpiderUUID(String uuid, Boolean includeRaw, OutputStream outputStream) {
		exportWebpageJSONBy(QueryBuilders.matchQuery("spiderUUID", uuid).operator(Operator.AND), includeRaw,
				outputStream);
	}
	...
	public void exportWebpageJSONByDomain(String domain, Boolean includeRaw, OutputStream outputStream) {
		exportWebpageJSONBy(QueryBuilders.matchQuery("domain", domain).operator(Operator.AND), includeRaw,
				outputStream);
	}
	...
	public Webpage getWebpageById(String id) {
		GetResponse response = client.prepareGet(INDEX_COMMONS, TYPE_WEBPAGE, id).get();
	...
	public boolean deleteById(String id) {
		DeleteResponse response = client.prepareDelete(INDEX_COMMONS, TYPE_WEBPAGE, id).get();
	...
	public List<Webpage> getWebpageByDomain(String domain, int size, int page) {
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(QueryBuilders.matchQuery("domain", domain)).addSort("gatherTime", SortOrder.DESC)
	...
	public List<Webpage> getWebpageByDomains(Collection<String> domain, int size, int page) {
		...
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(boolQueryBuilder).addSort("gatherTime", SortOrder.DESC).setSize(size)
				.setFrom(size * (page - 1));
		SearchResponse response = searchRequestBuilder.execute().actionGet();
		return warpHits2List(response.getHits());
	}
	...
	public List<Webpage> listAll(int size, int page) {
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(QueryBuilders.matchAllQuery()).addSort("gatherTime", SortOrder.DESC).setSize(size)
				.setFrom(size * (page - 1));
		SearchResponse response = searchRequestBuilder.execute().actionGet();
		return warpHits2List(response.getHits());
	}
	...
	public Pair<String, List<Webpage>> startScroll(QueryBuilder queryBuilder, int size) {
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(queryBuilder).setSize(size).setScroll(TimeValue.timeValueMinutes(SCROLL_TIMEOUT));
		SearchResponse response = searchRequestBuilder.execute().actionGet();
	...
	public boolean update(Webpage webpage) throws ExecutionException, InterruptedException {
		...
		updateRequest.index(INDEX_COMMONS);
		updateRequest.type(TYPE_WEBPAGE);
	...
	public Pair<Map<String, List<? extends Bucket>>, List<Webpage>> relatedInfo(String query, int size) {
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(QueryBuilders.queryStringQuery(query)).addSort("gatherTime", SortOrder.DESC)
				...
				.addAggregation(AggregationBuilders.terms("relatedKeywords").field("keywords")).setSize(size);
		SearchResponse response = searchRequestBuilder.execute().actionGet();
	...
	public Map<String, Long> countDomain(int size) {
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(QueryBuilders.matchAllQuery()).addAggregation(
						AggregationBuilders.terms("domain").field("domain").size(size).order(BucketOrder.count(false)));
		SearchResponse response = searchRequestBuilder.execute().actionGet();
	...
	public List<Webpage> moreLikeThis(String id, int size, int page) {
		MoreLikeThisQueryBuilder.Item[] items = { new MoreLikeThisQueryBuilder.Item(INDEX_COMMONS, TYPE_WEBPAGE, id) };
		String[] fileds = { "content" };
		SearchResponse response = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
				.setQuery(moreLikeThisQuery(fileds, null, items).minTermFreq(1).maxQueryTerms(12)).setSize(size)
				.setFrom(size * (page - 1)).execute().actionGet();
		return warpHits2List(response.getHits());
	}
	...
	public Map<Date, Long> countDomainByGatherTime(String domain) {
		AggregationBuilder aggregation = AggregationBuilders.dateHistogram("agg").field("gatherTime")
				.dateHistogramInterval(DateHistogramInterval.DAY).order(BucketOrder.key(false));
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(INDEX_COMMONS).setTypes(TYPE_WEBPAGE)
				.setQuery(QueryBuilders.matchQuery("domain", domain)).addAggregation(aggregation);
		SearchResponse response = searchRequestBuilder.execute().actionGet();
	...
	public boolean deleteByDomain(String domain, Task task) {
		return esUtil.deleteByQuery(QueryBuilders.matchQuery("domain", domain), task, INDEX_COMMONS, TYPE_WEBPAGE);
	}
	...
	public Pair<List<Webpage>, Long> getWebpageByKeywordAndDomain(String query, String domain, int size, int page) {
		...
		keyWorkQuery = QueryBuilders.queryStringQuery(query).analyzer("ik_max_word").defaultField("content");
		...
		searchRequestBuilder.setQuery(keyWorkQuery).setPostFilter(domainQuery).setSize(size).setFrom(size * (page - 1));
